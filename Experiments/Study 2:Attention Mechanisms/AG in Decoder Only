...
output_dir = f"/content/drive/MyDrive/4thyearproject_code/RESULTS/AG DECODER{timestamp}"
...
# Addition and integration of AG Block
# the attention gate, input and output are defined spatially 
class AttentionGate(nn.Module):
    def __init__(self, F_g, F_l, F_int):
        super(AttentionGate, self).__init__() #defining the gate and shrinking it
        self.W_g = nn.Sequential(
            nn.Conv3d(F_g, F_int, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm3d(F_int)
        )
        self.W_x = nn.Sequential( #defining the input and shrinking it
            nn.Conv3d(F_l, F_int, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm3d(F_int)
        )
        self.psi = nn.Sequential( #defining the addition of these two later on and what is going forward 
...
# here is where the new "value" is created which combines the gating feature.
...
# addition of the new gates before the decoder 
class UNetDecoderAG(nn.Module):   
    def __init__(self, in_channels, out_channels, channels=(16, 32, 64, 128, 256)):
        ...
        self.up4 = nn.ConvTranspose3d(channels[4], channels[3], kernel_size=2, stride=2) # upsampling
        self.g4 = AttentionGate(F_g=channels[3], F_l=channels[3], F_int=channels[3]//2) # gating
        self.dec4 = ConvBlock(channels[4], channels[3])

        self.up3 = nn.ConvTranspose3d(channels[3], channels[2], kernel_size=2, stride=2)
        self.g3 = AttentionGate(F_g=channels[2], F_l=channels[2], F_int=channels[2]//2)
        self.dec3 = ConvBlock(channels[3], channels[2])

        self.up2 = nn.ConvTranspose3d(channels[2], channels[1], kernel_size=2, stride=2)
        self.g2 = AttentionGate(F_g=channels[1], F_l=channels[1], F_int=channels[1]//2)
        self.dec2 = ConvBlock(channels[2], channels[1])

        self.up1 = nn.ConvTranspose3d(channels[1], channels[0], kernel_size=2, stride=2)
        self.g1 = AttentionGate(F_g=channels[0], F_l=channels[0], F_int=channels[0]//2)
    ...
    def forward(self, x):   
          ...
          up4 = self.up4(b) # upsampling decoderf features
          g4 = self.g4(up4, e4) # filtering the features 
          d4 = self.dec4(torch.cat([up4, g4], dim=1)) # concatenating and smoothing 

          up3 = self.up3(d4)
          g3 = self.g3(up3, e3)
          d3 = self.dec3(torch.cat([up3, g3], dim=1))

          up2 = self.up2(d3)
          g2 = self.g2(up2, e2)
          d2 = self.dec2(torch.cat([up2, g2], dim=1))

          up1 = self.up1(d2)
          g1 = self.g1(up1, e1)
          d1 = self.dec1(torch.cat([up1, g1], dim=1))
...
# Model Setup
model = UNetDecoderAG(in_channels=4, out_channels=NUM_CLASSES, channels=(16,32,64,128,256).to(DEVICE)
...
torch.save(model.state_dict(), os.path.join(output_dir, f"model2_study2_pretrained_final.pth"))
