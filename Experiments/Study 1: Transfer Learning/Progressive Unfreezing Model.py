...
output_dir = f"/content/drive/MyDrive/4thyearproject_code/RESULTS/PROGRESSIVE FREEZING_{timestamp}"
os.makedirs(output_dir, exist_ok=True)

# Utility
# This pads the images to multiples of 16 to make them suitable for the UNet
...
# Freezing function 
def freeze_encoder_blocks(model, level=4): 
    block_names = [f"down{i}" for i in range(level)]
    for name, param in model.named_parameters():
        if any(block in name for block in block_names):
            param.requires_grad = False
           
# Unfreezing fucntion
def unfreeze_encoder_block(model, level):
    block_name = f"down{level}"
    for name, param in model.named_parameters():
        if block_name in name:
            param.requires_grad = True

freeze_encoder_blocks(model, level=4)
...
for epoch in range(1, NUM_EPOCHS + 1):
    if epoch in [10, 20, 30, 40]: # the blocks are initially frozen then unfrozen when it reaches these epoch numbers 
        block_level = 4 - ((epoch - 10) // 10)
        unfreeze_encoder_block(model, level=block_level)
...
torch.save(model.state_dict(), os.path.join(output_dir, f"model3_study1_pretrained_final.pth"))
