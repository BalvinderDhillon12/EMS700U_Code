...
class CombinedLoss(nn.Module):
    def __init__(self):
        ...
        self.tversky = TverskyLoss(to_onehot_y=True, softmax=True, alpha=0.2, beta=0.8)

    def forward(self, inputs, targets):
         dice = 0.5*self.dice(inputs, targets)
         tversky = 0.5*self.tversky(inputs, targets)
        return dice(inputs, targets) + tversky(inputs, targets)
